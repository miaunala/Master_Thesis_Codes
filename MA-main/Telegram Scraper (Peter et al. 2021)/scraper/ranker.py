...
class Ranker:
    def __init__(self, step_size):
        ...
        with shelve.open('cache') as cache:
            ...
            print(f"{cache}")
            print(f"Cache keys {set(list(cache.keys()))}")
            if not {'scraped', 'ranking', 'channel_queue', 'iteration'}.issubset(set(list(cache.keys()))):
                self.iteration = 0
                self.ranking = {}
                print(f"Ranking: {self.ranking}")  # remove later
                self.channel_queue = {}
                print(f"Channel Queue: {self.channel_queue}")  # remove later
                self.scraped = {}
                print(f"Scraped: {self.scraped}")  # remove later
                self.removed_from_ranking = []
                print(f"Removed from Ranking: {self.removed_from_ranking}")  # remove later
                self.priority_channels = []
                print(f"Priority Channels: {self.priority_channels}")  # remove later
            else:
    ...
    def get_next_candidate(self):
        ...
        if len(self.channel_queue) == 0:
            self.__fill_queue()
            self.iteration += 1
        print(f"Next candidate: {max(self.channel_queue, key=self.channel_queue.get)}") # nachher löschen
    ...
    def set_candidate_scraped(self, channel_name):
        ...
        try:
            # print(f"set scraped candidate: {self.channel_queue[channel_name]}")
    ...
    def updateRanking(self, channel, curr_channel):
        ...
        for message in channel.messages:
            ...
            if forward_from is not None:
                print(f"current channel: {curr_channel}")
            ...
            for mention_link in message['mentioned_in_link']:
                print(f"Mention Link: {mention_link}")
    ...
    def set_seed_to_queue(self, channels):
        ...
        for channel in channels:
            print(f"Channel in channels that should be evaluated first: {channel}")
            channel_lower = channel.lower()
            self.channel_queue[channel_lower] = -1
            self.removed_from_ranking.append(channel_lower) # does it remove from ranking??????????
    ...
    def set_priority(self, channel_name):
        ...
        if channel_name not in self.removed_from_ranking:
            print(f"setting {channel_name} priority")
    ...
    def __cache_ranking(self):
        with shelve.open('cache') as cache:
            cache["ranking"] = self.ranking # DOING WEIRD STUFF; REPEATING CHANNELS IN CACHED RANKING; 82; TRY NEXT TIME WITH CACHE SETTING TO 0
            cache["channel_queue"] = self.channel_queue
            cache["scraped"] = self.scraped
            cache["iteration"] = self.iteration
            print(f"Cache Iteration: {cache['iteration']}")
