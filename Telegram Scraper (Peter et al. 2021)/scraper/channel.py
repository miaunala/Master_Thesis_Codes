...
class Channel:
    ...
    def __write_csv(self):
        ...
        print(self.username) # nachher wegmachen
    ...
    def __get_messages_formatted(self, username, min_id=0):
        try:
            print(self.username) # nachher löschen
            messages = list(self.client.get_messages(self.username, limit=self.num_messages, min_id=min_id))
            daterange = self.set_timeframe()

            messages_filter = list(filter(lambda msg: daterange[0] <= msg.date <= daterange[1], messages))
        ...
        return [Message(message, self.participants_count).__dict__ for message in messages_filter]
    ...
    def set_timeframe(self):
           utc = pytz.UTC
           # covid
           # corona begin_date = utc.localize(datetime.strptime("2020-03-01 00:00:00", "%Y-%m-%d %H:%M:%S"))
           # corona end_date = utc.localize(datetime.strptime("2021-03-01 00:00:00", "%Y-%m-%d %H:%M:%S"))
           # bauernproteste
           begin_date = utc.localize(datetime.strptime("2023-12-18 00:00:00", "%Y-%m-%d %H:%M:%S"))
           end_date = utc.localize(datetime.strptime("2024-07-22 20:00:00", "%Y-%m-%d %H:%M:%S"))
           timeframe = [begin_date, end_date]
           return timeframe
